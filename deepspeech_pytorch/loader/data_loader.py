...
class SpectrogramParser(AudioParser):
    ...
    def parse_audio(self, audio_path):
        ...
        D = torch.stft(torch.tensor(y), n_fft=n_fft, hop_length=hop_length,
...
class AttackDataset(Dataset, SpectrogramParser):
    def __init__(self,
                 audio_conf: SpectConfig,
                 input_path: str,
                 labels: list,
        ...
        self.size = len(self.ids)
        self.labels_map = dict([(labels[i], i) for i in range(len(labels))])
        super(AttackDataset, self).__init__(audio_conf, normalize, aug_cfg)
